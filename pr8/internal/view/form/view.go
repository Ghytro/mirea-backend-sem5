...
type View struct {
	controller form.UseCaseForm
}

func NewView(s form.UseCaseForm) *View {
	return &View{
		controller: s,
	}
}
...
func (a *View) getForms(c *fiber.Ctx) error {
	...
	forms, err := a.controller.GetForms(c.Context())
	if err != nil {
		return err
	}
	return c.JSON(forms)
...
func (a *View) addForm(c *fiber.Ctx) error {
	...
	form.UserId = user.Id
	if err := a.controller.AddForm(c.Context(), form); err != nil {
		return &entity.ErrResponse{
...
func (a *View) deleteForm(c *fiber.Ctx) error {
	...
	if err := a.controller.DeleteForm(c.Context(), user.Id, entity.PK(id)); err != nil {
		if err == pg.ErrNoRows {
			return &entity.ErrResponse{
				StatusCode: fiber.StatusBadRequest,
				Err: &entity.ServerError{
...
func (a *View) updateForm(c *fiber.Ctx) error {
	...
	if err := a.controller.UpdateForm(c.Context(), user.Id, &f); err != nil {
		return &entity.ErrResponse{
			StatusCode: fiber.StatusBadRequest,
			Err: &entity.ServerError{
				Message:   err.Error(),
